...
parser.add_argument('--FFC', '-r', action='store_true',default=False,help='FFC')
...
def test_abnormal(epoch):
    ...
    with torch.no_grad():
        for i, (data, data2) in enumerate(zip(anomaly_test_loader, normal_test_loader)):
            ...
            inputs2 = inputs2.view(-1, inputs2.size(-1))
        ...
        mean_tpr = np.mean([np.interp(mean_fpr, fpr, tpr) for fpr, tpr in zip(fpr_values, tpr_values)], axis=0)
        mean_auc = metrics.auc(mean_fpr, mean_tpr)
        
        if epoch==25 or epoch ==50:
            plot(mean_fpr, mean_tpr, mean_auc)
...
mean_tpr_final = np.mean([np.interp(mean_fpr_final, fpr, tpr) for fpr, tpr in zip(fp_final, tp_final)], axis=0)
mean_auc_final = metrics.auc(mean_fpr_final, mean_tpr_final)

plot(mean_fpr_final, mean_tpr_final, mean_auc_final)
